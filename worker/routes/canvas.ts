...
export async function handleCanvasRoute(
  ...
  try {
    ...
    if (parts.length === 2 && request.method === 'POST') {
      ...
      const timer = logger.startTimer('create-canvas');

      const canvas = await stub.getFullCanvas();

      timer.end({ canvasId });
      logger.info('Canvas created', { canvasId });
    ...
    logger.warn('Canvas route not found', { path: url.pathname });
    return jsonResponse({ error: 'Not found' }, 404, requestId);
  } catch (error) {
    logger.error('Canvas route failed', error, { path: url.pathname });
    return jsonResponse(
